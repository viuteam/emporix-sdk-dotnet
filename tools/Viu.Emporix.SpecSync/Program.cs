...
async Task<int> GenerateAsync()
{
    ...
    foreach (SpecSource spec in SpecCatalog.All)
    {
        ...
        IReadOnlyList<string> resolved = await PostProcessAsync(
            output,
            LocalizedProperties.Read(await File.ReadAllTextAsync(input)),
        LocalizedProperties.ReadUnions(await File.ReadAllTextAsync(input)));
...
/// </remarks>
static async Task<IReadOnlyList<string>> PostProcessAsync(
    string path,
    ...
    IReadOnlyCollection<string> unions)
{
    ...
    (content, IReadOnlyList<string> retyped, IReadOnlyList<string> missed) =
        GeneratedCodeFixer.RetypeLocalizedProperties(content, localized);
    (content, IReadOnlyList<string> untyped, IReadOnlyList<string> missedUnions) =
        GeneratedCodeFixer.RetypeUnionProperties(content, unions);
    ...
    if (untyped.Count > 0)
    {
        resolved = [.. resolved, .. untyped.Select(name => $"{name} → JsonElement (union)")];
    }

    if (missedUnions.Count > 0)
    {
        missed = [.. missed, .. missedUnions];
    }

    // Loud on purpose. A property the specification names and the generated code
    // does not have means one of the two moved.
    if (missed.Count > 0)
    {
        resolved =
        [
            .. resolved,
            .. missed.Select(name => $"NOT FOUND, left alone: {name}"),
        ];
    }
    ...
    // Reported as a count rather than a list: every enum in a specification is
    // annotated, which is dozens of names and no information.
    if (enums.Count > 0)
    {
