...
/// <summary>
/// Checks that every address the hand-written services build actually exists in
/// the Emporix specifications.
/// </summary>
/// <remarks>
/// <para>
...
/// never there. Every one of them compiled, and their tests passed, because the
/// tests asserted the same wrong path the code built.
/// </para>
/// <para>
...
{
    [Fact]
    public void Every_path_a_service_builds_exists_in_a_specification()
    {
        DirectoryInfo root = FindRepositoryRoot();
        HashSet<string> declared = ReadSpecificationPaths(root);
        List<(string Path, string File)> used = ReadServicePaths(root);
        ...
        string[] unknown = [.. used
            .Where(u => !declared.Contains(u.Path))
            .Select(u => $"{u.Path}  ({u.File})")
            .Distinct()
            .Order()];
    ...
    private static HashSet<string> ReadSpecificationPaths(DirectoryInfo root)
    {
        ...
        foreach (string file in Directory.EnumerateFiles(Path.Combine(root.FullName, "specs"), "*.yml"))
        {
            ...
            foreach (string line in File.ReadLines(file))
            {
                ...
                if (path.Success)
                {
                    paths.Add(Normalise(prefix + path.Groups[1].Value));
                }
            }
        }
    ...
    /// </summary>
    private static List<(string Path, string File)> ReadServicePaths(DirectoryInfo root)
    {
            ...
            "*.cs"))
        {
                ...
                RegexOptions.Singleline))
            {
                string built = string.Concat(
                    Regex.Matches(match.Groups[1].Value, @"""([^""]*)""").Select(m => m.Groups[1].Value));
